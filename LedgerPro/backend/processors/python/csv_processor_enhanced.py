            ...
            for row_dict in reader:
                ...
                for col_variant in self.forex_columns["original_amount"]:
                    matching_header = next(
                        (h for h in headers if h.lower() == col_variant.lower()), None
                    )
                    if matching_header:
                ...
                if forex_mapping.get("original_currency"):
                    currency = row_dict.get(
                        forex_mapping["original_currency"], ""
                    ).strip()
                    if currency and currency != "USD":
                        ...
                        if forex_mapping.get("original_amount"):
                            orig_amt_str = row_dict.get(
                                forex_mapping["original_amount"], ""
                            ).strip()
                            if orig_amt_str:
                                transaction["original_amount"] = (
                                    self.parse_amount_enhanced(orig_amt_str)
                                )

                        # Add exchange rate if available
                        if forex_mapping.get("exchange_rate"):
                            rate_str = row_dict.get(
                                forex_mapping["exchange_rate"], ""
                            ).strip()
                            if rate_str:
                                try:
                        ...
                        if not transaction.get("exchange_rate") and transaction.get(
                            "original_amount"
                        ):
                            if transaction["original_amount"] != 0:
                                transaction["exchange_rate"] = abs(
                                    transaction["amount"]
                                    / transaction["original_amount"]
                                )
