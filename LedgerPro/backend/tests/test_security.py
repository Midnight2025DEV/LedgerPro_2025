...
class TestSecurityFeatures:
        ...
        request_metrics.update(
            {
                ...
                "hourly_stats": defaultdict(
                    lambda: {"requests": 0, "uploads": 0, "errors": 0}
                ),
            }
        )
    ...
    def test_rate_limiting_upload_endpoint(self):
        ...
        for i in range(15):  # More than the 10/minute limit
            response = client.post(
                "/api/upload",
                files={
                    "file": (f"test_{i}.csv", io.BytesIO(small_content), "text/csv")
                },
            )
    ...
    def test_metrics_endpoint_requires_auth(self):
        ...
        response = client.get(
            "/api/metrics", headers={"Authorization": "Bearer invalid_token"}
        )
        assert response.status_code == 401
